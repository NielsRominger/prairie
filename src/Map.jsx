...
function Map() {
  ...
  const graph = {
    'starting-point': { 'top-right':1 , 'center-right':2, 'top-center': 5, 'top-left': 10, 'top-middle': 7 },
    'top-right': { 'starting-point':1, 'top-center':2 },
    'top-left': { 'starting-point':10, 'top-middle':4, 'center-left':7 },
    ...
    'bottom-right': { 'center-right':2, 'bottom-center':2, 'treasure':5, 'middle-center':6 },
  ...
  function dijkstra(graph, start) {
    ...
    while (nodes.length) {
      // Trier les noeuds par distance et sélectionner le plus proche
  ...
  // Appeler la fonction Dijkstra pour obtenir les distances et le chemin précédent
  ...
  path.reverse(); // Inverser le chemin pour qu'il aille de 'starting-point' à 'treasure'

  useEffect(() => {
    // Répertorier les positions de tous les points au chargement
  ...
  return (
    <div>
      ...
      {positions.map((pos, index) => (
        <p key={index}>
          Position de {points[index].name}: {pos ? `Top: ${pos.top}px, Left: ${pos.left}px` : 'Non disponible'}
        </p>
      ))}
    </div>
  );
